...
func FilterScopes(params []string) ScopeManager {
	...
	if utils.SliceContains(params, constants.IP6) {
		v6 = true
		dual = false
	}
	if (utils.SliceContains(params, constants.IP4) && utils.SliceContains(params, constants.IP6)) || utils.SliceContains(params, constants.DUAL) {
		v4 = false
		v6 = false
		dual = true
	}
	if utils.SliceContains(params, constants.URL) {
		v4 = false
		v6 = false
		dual = false
		url = true
	...
	if utils.SliceContains(params, constants.STELLAR) {
		scopes = append(scopes, constants.STELLAR)
	}
	if utils.SliceContains(params, constants.ORION) {
...
func MatchText(params []string) string {
	...
	if s.URL {
		if utils.SliceContains(s.Scopes, constants.STELLAR) {
			matched = append(matched, constants.DOMAINS_ST_CORP)
		}
		if utils.SliceContains(s.Scopes, constants.ORION) {
